  ...
  return (
    <section id="about" className="section-padding bg-card relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-5 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-48 sm:w-96 h-48 sm:h-96 bg-accent rounded-full blur-3xl" />
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-6">
            {stats.map((stat, index) => (
              <div 
                key={stat.label}
                className="bg-gradient-card p-4 sm:p-6 rounded-xl border border-border/50 hover:border-primary/50 transition-all duration-300 hover:shadow-[0_0_30px_hsla(0,85%,50%,0.2)] group"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <stat.icon className="w-6 h-6 sm:w-8 sm:h-8 text-primary mb-2 sm:mb-4 group-hover:scale-110 transition-transform duration-300" />
                <div className="text-2xl sm:text-3xl md:text-4xl font-heading font-bold text-foreground mb-1">
                  {stat.value}
                </div>
                <div className="text-xs sm:text-sm text-muted-foreground font-heading tracking-wide uppercase">
                  {stat.label}
                </div>
              </div>
            ))}
